...
// ✅ 현재 "타이핑 중 토큰" (2글자↑부터 추천)
const currentToken = useMemo(() => {
  const t = (draft.details || "");
  const m = t.match(/([0-9A-Za-z가-힣-]{2,})$/);
  return m ? m[1] : "";
}, [draft.details]);
...
// ✅ 단어 경계(공백/엔터/구두점)에서만 "확정 태그" 자동 추가
...
// token 안에 포함된 태그들을 뽑고, 겹치면 "가장 긴 것"만 남김
function pickCommittedTagsFromToken(token: string): string[] {
  const tokenRaw = (token || "").trim();
  if (!tokenRaw) return [];

  const tokenLower = tokenRaw.toLowerCase();
  const tokenLoose = normalizeLoose(tokenRaw);
...
function commitTags(tags: string[]) {
  ...
  const existing = new Set(
    next
      ? next.split(",").map((x) => x.trim()).filter(Boolean)
      : []
  );
...
useEffect(() => {
  ...
  // 공백/구두점 직전의 "마지막 토큰" (1글자도 허용: a 같은 태그 커밋 가능)
  ...
  commitTags(tags);
}, [draft.details, candidatePool, selectedTags]); // candidatePool/selectedTags는 최신 상태 반영용

// ✅ 타이핑 중 추천(가벼운 계산): currentToken만 대상으로
const allSug = useMemo(() => {
  if (!currentToken) return [];
  return buildSuggestions(currentToken, selectedTags, candidatePool, dismissed);
