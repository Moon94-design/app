...
type Partner = {
  id: string; name: string; address: string; status: "거래중" | "보류" | "중단";
  tags: string[]; contacts: any[]; vehicles: any[]; prices: PriceRow[];
...
type DailyLogisticsLine = LogisticsLine & { createdAt: string; createdBy: string };
...
const KEY_DRAFT = "draft_logistics_v1";
...
type Draft = { author: string; line: LogisticsLine };
...
export default function RegisterLogisticsDaily() {
  ...
  const vehicles = useMemo(() => loadJson<Vehicle[]>(KEY_VEHICLES, [] as Vehicle[]), []);
  ...
  const initDraft = useMemo(() => {
    const d = loadJson<Draft>(KEY_DRAFT, { author: localStorage.getItem(KEY_AUTHOR) || "", line: defaultLine() });
    // line.id는 페이지마다 새로여도 되므로 유지
    return d;
  }, []);
  ...
  function persist(nextAuthor: string, nextLine: LogisticsLine) {
    ...
    saveJson(KEY_DRAFT, { author: nextAuthor, line: nextLine });
  }
  ...
  function submit() {
    ...
    repo.logisticsLines().setAll(nextLines);
    ...
    repo.priceEvents().setAll([ev, ...prevEvents]);

    const partnersNext = partners.map((p) => {
      if (p.id !== saved.partnerId) return p;

      let nextPrices = Array.isArray(p.prices) ? [...p.prices] : [];
      const idx = nextPrices.findIndex((r) => r.direction === saved.direction && r.kind === saved.kind && r.item === saved.item);
      if (idx >= 0) nextPrices[idx] = { ...nextPrices[idx], pricePerKg: saved.unitPricePerKg };
      else nextPrices.push({ id: newId(), direction: saved.direction, kind: saved.kind, item: saved.item, pricePerKg: saved.unitPricePerKg });

      return { ...p, prices: nextPrices, updatedAt: nowIso() };
    });

    setPartners(partnersNext);
    repo.partners().setAll(partnersNext);
  ...
  return (
    <div className="card">
      ...
      <div className="card" style={{ background:"rgba(255,255,255,0.02)" }}>
        <div style={{ marginTop:12, display:"grid", gap:10 }}>
          ...
          <div>
            ...
            <div className="row" style={{ marginTop:8 }}>
              {(["매입","출고"] as const).map((d) => (
                <button key={d} type="button" className={`selBtn ${line.direction===d?"active":""}`} onClick={() => update({ direction: d })}>{d}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="row" style={{ marginTop:8 }}>
              {kinds.map((k) => (
                <button key={k} type="button" className={`selBtn ${line.kind===k?"active":""}`} onClick={() => update({ kind: k })}>{k}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="row" style={{ marginTop:8 }}>
              {(["PP","PE"] as const).map((it) => (
                <button key={it} type="button" className={`selBtn ${line.item===it?"active":""}`} onClick={() => update({ item: it })}>{it}</button>
              ))}
            </div>
          </div>

          <div><div className="p" style={{ marginTop:0 }}>수량(Kg)</div><input className="input" inputMode="numeric" value={String(line.qtyKg)} onChange={(e) => update({ qtyKg: Number(e.target.value || 0) })} /></div>
          <div><div className="p" style={{ marginTop:0 }}>단가(원/Kg)</div><input className="input" inputMode="numeric" value={String(line.unitPricePerKg)} onChange={(e) => update({ unitPricePerKg: Number(e.target.value || 0) })} /></div>
          <div><div className="p" style={{ marginTop:0 }}>총금액(자동)</div><input className="input" value={String(line.totalAmount)} readOnly /></div>
        </div>
      ...
      <div className="card" style={{ background:"rgba(255,255,255,0.02)" }}>
        ...
        {savedLines.length === 0 ? <p className="p">아직 없음</p> : savedLines.slice(0,20).map((s) => (
          <div key={s.id} className="card" style={{ marginTop:10, background:"rgba(255,255,255,0.02)" }}>
            ...
            <div className="p" style={{ marginTop:6 }}>{s.direction} · {s.kind} · {s.item} · {Math.round(s.unitPricePerKg).toLocaleString()}원/Kg</div>
