...
import { DAILY_BRANCH_OPTIONS, DAILY_PRODUCT_ITEM_OPTIONS } from "@kernel/schema/daily";
...
export const CATEGORY_OPTIONS: ProductCategory[] = [...DAILY_PRODUCT_ITEM_OPTIONS];
...
export function makeLogisticsDocId(recordDate: string, siteCode: string, actorKey: string): string {
  return `LOG_${recordDate}_${toSafeKeyPart(siteCode)}_${toSafeKeyPart(actorKey)}`;
}

export function makeLogisticsLegacyDocId(recordDate: string, siteCode: string, writerName: string): string {
  return `LOG_${recordDate}_${toSafeKeyPart(siteCode)}_${toSafeKeyPart(writerName)}`;
}
