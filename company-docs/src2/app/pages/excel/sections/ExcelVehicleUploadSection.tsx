...
type Props = {
  ...
  onApply: (result: VehicleParseResult) => void;
};

export default function ExcelVehicleUploadSection({ existingVehicleNos, onApply }: Props) {
    ...
    <div className="card" style={{ background: "rgba(255,255,255,0.02)", padding: 20, marginBottom: 20 }}>
      ...
      {parseResult && (
        <ExcelParseResultView
          stats={{ total: parseResult.total, ok: parseResult.ok, incomplete: parseResult.incomplete, fail: parseResult.fail }}
          ...
          policyMessage={
            <>
              ...
              <div className="p" style={{ marginTop: 4, fontSize: 13 }}>
                • 엑셀에는 운송사/기사명/연락처가 없어서 관리 페이지에서 추가 보완이 필요합니다.
              </div>
              <div className="p" style={{ marginTop: 4, fontSize: 13 }}>
                • 적용 예정: <strong>{applyCount}건</strong>
              </div>
            </>
          }
          ...
          previewColumns={[
            ...
            { header: "상태", key: "status", render: (row: VehicleParsedRow) => renderStatusBadge(row.status) },
