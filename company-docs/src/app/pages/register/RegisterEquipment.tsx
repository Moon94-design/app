...
type Equipment = {
  ...
  makerModel: string;
  installedAt: string; // YYYY-MM-DD(텍스트)
...
export default function RegisterEquipment() {
  // ✅ repo에서 읽기
  const [equipments, setEquipments] = useState<Equipment[]>(() => repo.equipments<Equipment>().getAll());
  const [consumables, setConsumables] = useState<Consumable[]>(() => repo.consumables<Consumable>().getAll());
  const vendors = useMemo(() => repo.vendors<Vendor>().getAll(), []);
  ...
  function submitEquipment() {
    ...
    setEquipments(next);
    repo.equipments<Equipment>().setAll(next);
  ...
  function addConsumableFromEquipment() {
    ...
    repo.consumables<Consumable>().setAll(consNext);
    ...
    repo.equipments<Equipment>().setAll(eqNext);
    ...
    <div className="card">
      ...
      <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
        ...
        <div>
          ...
          <input className="input" inputMode="numeric" value={draft.installedAt} onChange={(e) => persist({ ...draft, installedAt: e.target.value.replace(/[^0-9-]/g, "").slice(0, 10) })} placeholder="YYYY-MM-DD" />
        </div>
